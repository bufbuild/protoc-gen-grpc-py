...
def reply(message: str) -> tuple[str, bool]:
    ...
    if message in _GOODBYE_INPUTS:
        return random.choice(_GOODBYE_RESPONSES), True  # noqa: S311
...
def get_intro_responses(name: str) -> Sequence[str]:
    ...
    intros.append(random.choice(_ELIZA_FACTS))  # noqa: S311
...
def _lookup_response(message: str) -> str:
    for pattern, responses in _REQUEST_INPUT_REGEX_TO_RESPONSE_OPTIONS.items():
        ...
        if not match:
            continue
        response = random.choice(responses)  # noqa: S311
        if "{}" not in response:
            return response

        fragment = _reflect(match.group(1))
        return response.format(fragment)
    return random.choice(_DEFAULT_RESPONSES)  # noqa: S311
...
def _reflect(fragment: str) -> str:
    ...
    for i, word in enumerate(words):
        if reflected := _REFLECTED_WORDS.get(word):
            words[i] = reflected
    return " ".join(words)
...
_REQUEST_INPUT_REGEX_TO_RESPONSE_OPTIONS: dict[re.Pattern[str], list[str]] = {
    re.compile(r"i need (.*)"): [
        "Why do you need {}?",
...
# A string array of facts about ELIZA. Used in responses to Introduce, which is
# a server-stream.
